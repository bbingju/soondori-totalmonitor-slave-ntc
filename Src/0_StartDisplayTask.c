...
GPIO_TypeDef *SLOT_EN[4] = {SLOT_EN_00_GPIO_Port, SLOT_EN_01_GPIO_Port,
                            SLOT_EN_02_GPIO_Port, SLOT_EN_03_GPIO_Port};
uint16_t SLOT_EN_PIN[4] = {SLOT_EN_00_Pin, SLOT_EN_01_Pin, SLOT_EN_02_Pin,
                           SLOT_EN_03_Pin};
...
void StartDisplayTask(void const *argument)
{
    ...
    for (;;) {
            ...
            if (TestData.overTempFlag[i] == TM_NORMAL_TEMP) {
                ...
                } else if (TestData.AdcMidValue[0][i].UI32 < 10) //센서 쇼트됨
                {
                    if (TestData.displayModeFlag[i] == LDM_SENSOR_CLOSE) {
                        doLedFlickingDisplay(i, (overTempDisplay % 4),
                                             2); // led 고속 점멸
                    } else {
                        TestData.displayModeChangeCount[i]++;
                        if (TestData.displayModeChangeCount[i] > 3) {
        ...
        // SYSTEM RESET
        /*        if(HAL_GPIO_ReadPin(SLOT_EN[SysProperties.boardID - '0'],
           SLOT_EN_PIN[SysProperties.boardID - '0']) == GPIO_PIN_RESET)
                  {
