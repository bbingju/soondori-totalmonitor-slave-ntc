...
void doTempReq()
{
    util_mem_cpy(&temp[0], &TestData.Temperature[0][0].Float, 128);

	doMakeSendTempData(transmtData, CMD_TEMP_REQ, &temp[0], 128, 134);
...
{
    util_mem_cpy(&temp[0], &TestData.displayModeFlag[0], 16);

	doMakeSendTempData(transmtData, CMD_TEMP_STATE_REQ, &temp[0], 16, 38);
...
void doTresholdSet(void)
{
    ...
    util_mem_cpy(&temp[0], &TestData.Threshold[0][0].UI8[0], 128); 
...
void doRevisionApplySet(void)
{
	...
	util_mem_set(transmtData, 0x00, sizeof(transmtData));	
